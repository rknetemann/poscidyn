...
class NonlinearDynamics:
    ...
    def _steady_state_event(self, t, state, args, **kwargs) -> jax.Array:
        del kwargs
        t_theoretical_steady_state = self.model.get_steady_state_t_end()
    ...
    def frequency_response(
        ...
        if isinstance(self.model, PhysicalModel):
            if F_omega_hat or F_amp_hat or y0_hat or tau_end:
                raise ValueError("F_omega_hat, F_amp_hat, y0_hat, and tau_end are not supported for PhysicalModel.")
            
        if isinstance(self.model, NonDimensionalisedModel):
            if F_omega or F_amp or y0 or t_end:
                raise ValueError("F_omega, F_amp, y0, and t_end are not supported for NonDimensionalisedModel.")
