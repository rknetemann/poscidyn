...
C
               DO I=1,NDIM
                  IF(IREV(I).GT.0) THEN
                   IF (IREV(I).EQ.1) THEN
c *****NOTE MODIFICATION FROM GENERAL CASE
                       FB(JB)=DSIN(U1(I))
C                        FB(JB)=U1(I)
                   ELSE
                       FB(JB)=U1(I)  
...
C        *Explicit boundary conditions for homoclinic orbit at t=0
         ...
         IF(NUNSTAB.GT.1) THEN
            ...
            DO J=1,NUNSTAB
               DO I=1,NDM
                  FB(I)=U0(I)-XEQUIB1(I)-PAR(IP+J)*VR(NSTAB+J,I)
               ENDDO
               DUM=DUM+PAR(IP+J)**2
            ENDDO
	    JB=NDM+1
...
C     *regular continuation
        ...
        ELSE
          ICORR = 1
        ENDIF
        NBC=NSTAB+NUNSTAB+(ITWIST+IEQUIB)*NDM+NFREE-NINT-ICORR
        IF (IEQUIB.EQ.2) THEN
	  NBC=NBC-NDM+1
        ENDIF
        IF (IEQUIB.LT.0) THEN
...
C
C     ---------- -----
      SUBROUTINE PREHO(NDX,NTSR,NAR,NDIM,NCOLRS,UPS,UDOTPS,TM,PAR)
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
...
C
      DIMENSION UPS(NDX,*), TM(*), UDOTPS(NDX,*), PAR(*)
      COMMON /BLHOM/ ITWIST,ISTART,IEQUIB,NFIXED,NPSI,NUNSTAB,NSTAB,NREV
C      
      ...
      ENDIF
C
C Shift phase if necessary if continu(e)ing from
C a periodic orbit into a homoclinic one
C
      IF (ISTART.EQ.1) THEN
C
C First find smallest value in norm
C
       UPSMIN=1D20
       JMIN=1
       DO J=1,NTSR+1
...
C		CALL STPNT(NDM,U,PAR)  ** Marco changed **
C
...
C
 7    CONTINUE
      ...
      RETURN
C
C Three leading eigenvalues (ustable)
C
 8    CONTINUE
      PSIHO=RR(NSTAB+1)-RR(NUNSTAB+3)
      RETURN
C
