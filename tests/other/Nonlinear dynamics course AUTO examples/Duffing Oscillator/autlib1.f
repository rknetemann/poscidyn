...
C
c	
C	KOSTAS CHANGED LINES 2107 AND 1416
      INCLUDE 'auto.h'
C
...
C
       ...
       OPEN(7,ACCESS='APPEND',FILE='fort.7',STATUS='unknown')
       OPEN(8,ACCESS='APPEND',FILE='fort.8',STATUS='unknown')
       OPEN(9,FILE='fort.9',STATUS='unknown')
	 OPEN(6,ACCESS='APPEND',FILE='fort.6',STATUS='unknown')
C
 1     T0=DCLOCK()
       CALL AUTIM0(TIME0)
       FOUND=.FALSE.
...
C        ** Fold continuation (Periodic solutions, restart).
     ...
     *   PVLSBV,THL,THU,IUZ,VUZ)
C
       ELSE IF(IPS.EQ.2 .AND. IABS(ISW).EQ.2 .AND. ITP.EQ.7 ) THEN
C        ** Continuation of period doubling bifurcations (start).
         CALL AUTOBV(IAP,RAP,PAR,ICP,FNPD,BCPD,ICPD,STPNPD,
     *      PVLSBV,THL,THU,IUZ,VUZ)
C
       ELSE IF(IPS.EQ.2 .AND. IABS(ISW).EQ.2 .AND. (IABS(ITP)/10).EQ.7)
     * THEN
...
C            ** Variable period
           ...
           ICP(4)=12
C
         ELSE IF(ITP.EQ.7 .AND. IPS.EQ.2)THEN
C          ** Continuation of period doubling bifurcations; start
           NDIM=2*NDIM
           NBC=NDIM
           NINT=2
...
C            ** Fixed period
           ...
           IF(IAP(38).EQ.0)WRITE(6,101)
C
         ELSE IF(IABS(ITP)/10.EQ.7 .AND. IPS.EQ.2)THEN
C          ** Continuation of period doubling bifurcations; restart
           NDIM=2*NDIM
           NBC=NDIM
           NINT=2
...
C
       NDIM=IAP(1)
C
C       CALL STPNT(NDIM,U,PAR)  ** marco changed **
...
C
 1     DSOLD=RDS
       ...
       IAP(31)=NIT
       NTOT=IAP(32)
       NTOP=MOD(NTOT+1,10000)
       NDMR=NDIM
c    
C      Kostas changed the following line 1416 to print more
c       IF(NDMR.GT.6)NDMR=6
       IF(IID.GE.2.AND.IAP(38).EQ.0)THEN
          CALL WRBAR(80)
          IF(NIT.EQ.0)WRITE(9,100)
          WRITE(9,101)IBR,NTOP,NIT,RLCUR(1),(U(I),I=1,NDMR)
       ENDIF
 100   FORMAT('  BR    PT  IT')
 101   FORMAT(I4,I6,I4,4X,1PE14.6,14X,1P6E14.6)
...
C
         IF(IID.GE.2.AND.IAP(38).EQ.0)THEN
            IF(IAP(38).EQ.0)WRITE(9,101)IBR,NTOP,
     *    NIT,RLCUR(1),(U(I),I=1,NDMR)
         ENDIF
C
...
C        Use Mueller's method with bracketing for subsequent steps
         ...
         GOTO 1
       ELSE
         IF(IAP(38).EQ.0)WRITE(9,103)IBR,MOD(NTOT+1,10000)
         Q=0.d0
         RETURN
       ENDIF
C
...
C If requested write additional output on unit 9 :
C
       IF(IID.GE.2.AND.IAP(38).EQ.0)WRITE(9,101)IBR,NTOP,FNBPAE
 101   FORMAT(I4,I6,8X,'BP   Function ',1PE14.6)
C
      RETURN
      END
C
...
C If requested write additional output on unit 9 :
C
       IF(IID.GE.2.AND.IAP(38).EQ.0)WRITE(9,101)IABS(IBR),NTOP,FNLPAE
 101   FORMAT(I4,I6,8X,'Fold Function ',1PE14.6)
C
      RETURN
      END
C
       ...
       DO I=1,NDM
         IF(DIMAG(EV(I)).NE.0.d0)THEN
           ...
           IF(AR.LE.AREV)THEN
             ...
             IF(RIMHB.NE.0.d0)PAR(11)=PI(2.d0)/RIMHB
           ENDIF
         ENDIF
       ENDDO
C
C Compute the number of eigenvalues with negative real part.
C
       NINS1=0
       IF(ISW.NE.2)THEN
         DO I=1,NDM
           IF(DREAL(EV(I)).LE.0.d0)NINS1=NINS1+1
         ENDDO
       ELSE
         DO I=1,NDM
           IF(DREAL(EV(I)).LE.HMACH)NINS1=NINS1+1
         ENDDO
       ENDIF
C
       IF(ISW.EQ.2)THEN
...
C
       NTOT=IAP(32)
       NTOTP1=NTOT+1
       IF(IID.GE.2.AND.IAP(38).EQ.0)WRITE(9,101)IABS(IBR),NTOP,FNHBAE
       IF(NINS1.EQ.NDM)NTOTP1=-NTOTP1
C
       IF(IAP(38).EQ.0)THEN
         WRITE(9,102)IABS(IBR),NTOP,NINS
         IF(IPS.EQ.-1)THEN
           DO I=1,NDM
             WRITE(9,103)IABS(IBR),NTOP,I,EXP(EV(I))
           ENDDO
         ELSE
           DO I=1,NDM
             WRITE(9,103)IABS(IBR),NTOP,I,EV(I)
           ENDDO
         ENDIF
       ENDIF
C
 101   FORMAT(I4,I6,8X,'Hopf Function ',1PE14.6)
 102   FORMAT(I4,I6,8X,'Eigenvalues:',33X,'Stable:',I3)
 103   FORMAT(I4,I6,8X,'Eigenvalue',I3,1X,1P2E14.6)
C
      RETURN
      END
C
...
C
       ...
       CHNG=.TRUE.
C
       IF(IID.GE.3)WRITE(9,101)IABS(IBR),NTOP,IUZR,FNUZAE
 101   FORMAT(I4,I6,8X,'User Func.',I3,1X,1PE16.6)
C
      RETURN
      END
C
...
c	
C       Kostas changed the following line 2107 to print more values
C       IF(NDMR.GT.6)NDMR=6
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
C Prints headings above columns on unit 6 and 7.
C
      DIMENSION PAR(*),ICP(*),IAP(*)
      CHARACTER*15 COL(165),TMP
      CHARACTER*1 CNVRT
	CHARACTER*2 CNVRTMA
...
C
 100   FORMAT(' ')
 101   FORMAT('   0')
 102   FORMAT('  BR    PT  TY LAB ',62A15)
 103   FORMAT('   0    PT  TY LAB ',62A15)
C
      RETURN
      END
C
...
C      Marco made it
      CHARACTER*2 FUNCTION CNVRTMA(I)
C
       CNVRTMA='  '
C
     ...
     & 83,84,85,86,87,88,89,90,91)I+1
 1     CNVRTMA=' 0'
      RETURN
 2     CNVRTMA=' 1'
      RETURN
...
C
       ...
       NT=N1+N2
C
c       IF(N1.GT.7)THEN    ** marco changed **
c         N1=7
c         N2=0
c       ELSEIF(NT.GT.7)THEN    ** marco changed **
c         N2=7-N1
c       ENDIF
C
...
C
     ...
     *     (U(I),I=1,NDIM)
           WRITE(7,102)IBR,MTOT,ITP,LAB,PAR(ICU(1)),VAXIS,(U(I),I=1,NDIM) 
           WRITE(9,101)IBR,MTOT,ATYPE,LAB,PAR(ICU(1)),VAXIS,
     *     (U(I),I=1,N2)
         ELSE
           IF(MOD(ITP,10).NE.0)
     *     WRITE(6,101)IABS(IBR),IABS(MTOT),ATYPE,LAB,PAR(ICU(1)),pipo,
     *     (U(I),I=1,NDIM),(PAR(ICU(I)),I=2,N1)
           WRITE(7,102)IBR,MTOT,ITP,LAB,PAR(ICU(1)),pipo,
     *     (U(I),I=1,NDIM),(PAR(ICU(I)),I=2,N1)
           WRITE(9,101)IBR,MTOT,ATYPE,LAB,PAR(ICU(1)),pipo,
     *     (U(I),I=1,N2),(PAR(ICU(I)),I=2,N1)
         ENDIF
       ENDIF
C
 101   FORMAT(I4,I6,2X,A2,I5,1P62E15.6e3,1P62E15.6e3,1PE15.6e3)
 102   FORMAT(I4,I6,I4,I5,1P62E15.6e3,1P62E15.6e3,1PE15.6e3)
C
      RETURN
      END
C
...
C
       WRITE(9,101)IABS(IBR),NTOP-1,NIT
       WRITE(9,102)IABS(IBR),NTOP,RDS
 101   FORMAT(I4,I6,8X,'Iterations     ',I3)
 102   FORMAT(I4,I6,8X,'Stepsize',6X,1PE14.6)
C
      RETURN
      END
C
...
C
      COMPLEX*16 EV(*)
C Local
      DIMENSION WR(NDIMX),WI(NDIMX),Z(NDIMX,NDIMX)
      DIMENSION FV1(NDIMX)
      INTEGER   IV1(NDIMX)
C
       IBR=IAP(30)
...
C
         IF(PIV.LT.RSMALL.AND.IAM.EQ.0)WRITE(9,101)IBR,NTOP,JJ,RSMALL
C
         K=IR(JJ)
         IR(JJ)=IR(IPIV)
         IR(IPIV)=K
...
C
      DIMENSION THU(*)
C
C Finds the norm of UPS (first NDIM1 components are included only).
C
...
C
       DO J=1,NTSRS
         ...
         TM(J)=TEMP(1)
       ENDDO
       READ(3,101)TM(NTSRP1),(UPS(NTSRP1,K),K=1,NDIMRD)
       IF(NSKIP1.GT.0)CALL SKIP3(NSKIP1,EOF3)
C
       READ(3,*)(ICPRS(I),I=1,NFPRS)
       READ(3,101)(RLDOT(I),I=1,NFPRS)
C
...
C
       DO J=1,NTSRS
         ...
         ENDDO
       ENDDO
       READ(3,101)(UDOTPS(NTSRP1,K),K=1,NDIMRD)
       IF(NSKIP2.GT.0)CALL SKIP3(NSKIP2,EOF3)
C
...
C
       IF(IPS.EQ.9)THEN
         CALL PREHO(NDX,NTSRS,NDIMRD,NDIM,NCOLRS,UPS,UDOTPS,TM,PAR)
C
C Special case : Preprocess restart data in case of branch switching
C at a period doubling bifurcation.
C
       ELSE IF((IPS.EQ.2.OR.IPS.EQ.6).AND.ISW.EQ.-1.AND.ITPRS.EQ.7) THEN
...
C
       DO J=1,NTST+1
         ...
         DO I=1,NCOL1
           ...
           K1=(I-1)*NDIM+1
           K2=I*NDIM
C          CALL STPNT(NDIM,U,PAR)   ** Kostas changed it **
           CALL STPNT(NDIM,U,PAR,T)
           DO K=K1,K2
             UPS(J,K)=U(K-K1+1)
           ENDDO
...
C
         CALL SCALEB(IAP,ICP,NDX,UDOTPS,RLDOT,DTM,THL,THU)
         IF(IID.GE.2.AND.IAP(38).EQ.0)THEN
           WRITE(9,101)IABS(IBR),NTOP,RLDOT(1)
         ENDIF
C
...
C
      IF(IAP(38).GT.0)RETURN
C
      IF(IID.GE.2)WRITE(9,101)IABS(IBR),NTOP,FNBPBV
 101  FORMAT(I4,I6,8X,'BP   Function ',1PE14.6)
C
      RETURN
      END
...
C
       ...
       IF(AMIN.GT.5.0E-2 .AND. ISP.EQ.2) THEN
         IF(IAP(38).EQ.0)THEN
            IF(IID.GE.2)WRITE(9,101)IABS(IBR),NTOP
            DO I=1,NDIM
              WRITE(9,105)IABS(IBR),NTOP,I,EV(I)
            ENDDO
         ENDIF
         NINS=0
         IAP(33)=NINS
         IF(IAP(38).EQ.0)WRITE(9,104)IABS(IBR),NTOP,NINS
         ISP=-ISP
         IAP(9)=ISP
         RETURN
       ENDIF
...
C
       IF(ISP.LT.0)THEN
         IF(AMIN.LT.1.0E-2)THEN
           IF(IAP(38).EQ.0)WRITE(9,102)IABS(IBR),NTOP
           ISP=-ISP
           IAP(9)=ISP
         ELSE
           IF(IAP(38).EQ.0)THEN
...
C
       ...
       ELSE
         NINS1=1
         DO I=2,NDIM
           IF( ABS(EV(I)).LE.1.d0)NINS1=NINS1+1
         ENDDO
         IF(ISP.EQ.2) THEN
...
C
       ...
       IF( IID.GE.2 .AND. (ISP.EQ.1 .OR. ISP.EQ.2) )THEN
          IF(IAP(38).EQ.0)WRITE(9,103)IABS(IBR),NTOP,D
       ENDIF
C
...
C
       NINS=IAP(33)
       IF(IAP(38).EQ.0)THEN
          WRITE(9,104)IABS(IBR),NTOP,NINS
          DO I=1,NDIM
            WRITE(9,105)IABS(IBR),NTOP,I,EV(I)
          ENDDO
       ENDIF
C
 ...
 103   FORMAT(I4,I6,8X,'SPB  Function ',1PE14.6)
 104   FORMAT(I4,I6,8X,'Multipliers:',33X,'Stable:',I3)
 105   FORMAT(I4,I6,8X,'Multiplier',I3,1X,1P2E14.6)
C
      RETURN
      END
C
...
C
       ...
       CHNG=.TRUE.
C
       IF(IID.GE.3)WRITE(9,101)IABS(IBR),NTOP,IUZR,FNUZBV
 101   FORMAT(I4,I6,8X,'User Func.',I3,1X,1PE14.6)
C
      RETURN
      END
C
...
C Local
C      DIMENSION UMX(7)  ** Marco changed **
C
       DIMENSION UMX(IAP(1))
...
C
       N2=NDM
c       IF(N2.GT.7)N2=7   ** Marco changed **
c       DO I=1,N2         ** Marco changed **
       DO I=1,NDIM
         ITMP=I
       ...
       IF(IID.GE.2)THEN
         ...
         IF(IAP(38).EQ.0)THEN
           IF(NITPS.EQ.0 .OR. IID.GE.3)THEN
             ...
             WRITE(9,102)
           ENDIF
           MTOT=MOD(NTOT+1,10000)
           WRITE(9,103)IBR,MTOT,NITPS,RLCUR(1),AMP
         ENDIF
       ENDIF
C
...
C
 102   FORMAT('  BR    PT  IT')
 103   FORMAT(I4,I6,I4,4X,1P6E14.6)
 104   FORMAT(' UPS :')
 105   FORMAT(1X,1P7E14.6)
C
      RETURN
      END
C
