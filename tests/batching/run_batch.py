...
def worker(gpu_id, task_q, result_q, driving_frequencies, driving_amplitudes):
    ...
    def _build_model(Q_val, gamma_val):
        Q_arr       = jnp.array([Q_val], dtype=oscidyn.const.DTYPE)
        omega_0_hat = jnp.array([1.0],   dtype=oscidyn.const.DTYPE)  # extend later if you add omega0 to file
    ...
    def _run_with_direction(Q_val, gamma_val, direction):
    ...
    run_forward  = jax.jit(lambda Q_val, gamma_val: _run_with_direction(Q_val, gamma_val, oscidyn.SweepDirection.FORWARD))
    run_backward = jax.jit(lambda Q_val, gamma_val: _run_with_direction(Q_val, gamma_val, oscidyn.SweepDirection.BACKWARD))
