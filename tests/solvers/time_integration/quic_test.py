...
def plot_sweep(axes, drive_freqs, drive_amps, sweeped_solutions, periodic_solutions, param_text: str) -> None:
    ...
    periodic_modes = np.asarray(periodic_solutions["max_x_modes"])
    if periodic_modes.ndim != 5:
        raise ValueError(
            f"Expected periodic solutions to have 5 dimensions, got shape {periodic_modes.shape}"
        )

    n_freq, n_amp, n_init_disp, n_init_vel, n_modes = periodic_modes.shape
    axes = np.atleast_1d(axes).ravel()
    if axes.size != n_modes:
        raise ValueError(f"Need {n_modes} axes to plot per-mode data, got {axes.size}.")

    drive_freqs = np.asarray(drive_freqs)
    drive_amps = np.asarray(drive_amps)
    if drive_freqs.shape[0] != n_freq:
        raise ValueError(
            f"Drive frequency axis mismatch: expected length {n_freq}, got {drive_freqs.shape[0]}"
        )
    if drive_amps.shape[0] != n_amp:
        raise ValueError(
            f"Drive amplitude axis mismatch: expected length {n_amp}, got {drive_amps.shape[0]}"
        )

    colors = plt.cm.viridis(np.linspace(0, 1, drive_amps.size))
    n_init_states = n_init_disp * n_init_vel
    ...
    for mode_idx, ax in enumerate(axes):
        for amp_idx, (amp, color) in enumerate(zip(drive_amps, colors)):
            ...
            mode_values = periodic_modes[:, amp_idx, :, :, mode_idx].reshape(
                n_freq, n_init_states
            )
            ax.scatter(
                np.repeat(drive_freqs, n_init_states),
                mode_values.ravel(),
                s=18,
                color=color,
                alpha=0.5,
        ...
        ax.set_title(f"Mode {mode_idx + 1} max displacement")
        ax.set_ylabel("Max displacement")
        ax.grid(alpha=0.25)

        if mode_idx == 0 and param_text:
            ax.text(
                0.02,
                0.98,
                param_text,
        ...
        style_labels = []
        if forward is not None:
            style_handles.append(Line2D([0], [0], color="k", linestyle="-", lw=1.5))
            style_labels.append("Forward sweep")
        if backward is not None:
            style_handles.append(Line2D([0], [0], color="k", linestyle="--", lw=1.5))
            style_labels.append("Backward sweep")
        style_handles.append(
            Line2D(
                ...
                linestyle="None",
                markersize=5,
                label="Periodic solutions",
            )
        )
        style_labels.append("Periodic solutions")
...
# 2 modes:
Q, omega_0, alpha, gamma = np.array([10.0, 10.0]), np.array([2.95, 3.97]), np.zeros((2,2,2)), np.zeros((2,2,2,2))
gamma[0,0,0,0] = 1.25e-3
gamma[1,1,1,1] = 2.50e-3
...
DRIVING_FREQUENCY = np.linspace(1.5, 8.0, 250)
DRIVING_AMPLITUDE = np.linspace(10.0, 40.0, 2)
...
n_modes = frequency_sweep.periodic_solutions["max_x_modes"].shape[-1]

end_time = time.time()
print(f"Frequency sweep completed in {end_time - start_time:.2f} seconds.")

fig_height = max(4.5, 3.0 * n_modes)
fig, axes = plt.subplots(n_modes, 1, figsize=(10, fig_height), sharex=True)
plot_sweep(
    axes=axes,
    ...
    periodic_solutions=frequency_sweep.periodic_solutions,
